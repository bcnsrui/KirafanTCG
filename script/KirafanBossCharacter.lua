...
function Kirafan7.BossCharacter(c)
	...
	Kirafan7.BossMainCharacterEff(c)
...
function Kirafan7.DuelSkipop(e,tp,eg,ep,ev,re,r,rp)	
	...
	if main:GetDefense()<=12 then
	local bg=Duel.GetDecktopGroup(1-tp,1)
	Duel.Sendto(bg,LOCATION_EXTRA,REASON_RULE,POS_FACEDOWN) end
...
function Kirafan7.BossMainCharacterEff(c)
	local e1=Effect.CreateEffect(c)
	...
	e8:SetValue(Kirafan7.fieldlevel)
	c:RegisterEffect(e8)
...
function Kirafan7.fieldlevel(e,c)
	local ct=Duel.GetFieldGroupCount(e:GetHandlerPlayer(),LOCATION_MZONE,0)-2
	local ct2=Duel.GetTurnCount()
	return ct+ct2+2
end
